-- RESET ONLY OWNER DRAWS - Precise fix to only affect owner draw transactions
-- This script ONLY affects transactions marked with is_owner_draw = TRUE
...
-- Step 3: Reset ONLY transactions marked as owner draws - NOTHING ELSE
...
-- Step 5: Verify what was reset - show ONLY owner draws
SELECT 
  COUNT(*) as total_owner_draws_reset,
  SUM(CASE WHEN is_cleared = FALSE THEN 1 ELSE 0 END) as now_uncleared,
  SUM(CASE WHEN is_reconciled = FALSE THEN 1 ELSE 0 END) as now_unreconciled
FROM bank_transactions
WHERE is_owner_draw = TRUE;

-- Summary
-- ONLY owner draw transactions (is_owner_draw = TRUE) have been:
